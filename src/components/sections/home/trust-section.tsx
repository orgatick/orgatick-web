    ...
    <section className="py-20 md:py-28 border-b border-border/40 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <h2 className="text-3xl sm:text-5xl font-extrabold text-foreground tracking-tight">Trusted by Top Student Bodies & Fest Committees</h2>
          ...
          {testimonials.map((t) => (
            ...
            >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-0.5 text-amber-500">
                    {Array.from({ length: 5 }, (_, i) => (
                      <IconStarFilled key={`star-${i + 1}`} className="size-4" />
                    ))}
                  </div>
              ...
              <div className="pt-4 border-t border-border/40 flex items-center gap-3">
                <div className="size-10 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-sm">{t.author.charAt(0)}</div>
                <div>
                  <h4 className="font-bold text-sm text-foreground">{t.author}</h4>
                  <p className="text-xs text-muted-foreground">
                    {t.title} <span className="font-semibold text-primary">{t.college}</span>
                  </p>
                </div>
              </div>
            </Card>
