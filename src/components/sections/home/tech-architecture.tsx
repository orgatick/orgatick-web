...
export function TechArchitectureSection() {
  const stack = [
    { layer: "Web Application", tech: "Next.js / React (SSR & Server Components)", desc: "Lightning fast page loads and seamless digital ticket rendering." },
    {
  ...
  const stateMachine = [
    ...
    { state: "03 Sales Open", desc: "Live payments, referral tracking & ticket issuance" },
    { state: "04 Check-in Active", desc: "Venue QR scanners active & low-latency gate stream" },
    { state: "05 Audited", desc: "Financial ledger settlement & participation certificates" },
  ];
    ...
    <section id="architecture" className="py-20 md:py-28 bg-muted/20 border-b border-border/40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <h2 className="text-3xl sm:text-5xl font-extrabold text-foreground tracking-tight">Built for Peak Festival Traffic</h2>
          <p className="text-muted-foreground text-base sm:text-lg">
            Orgatick is built on state-machine architecture to ensure transaction consistency, zero double-bookings, and sub-second venue check-in speeds.
          </p>
        </div>
        ...
        <div className="mt-16 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {stack.map((item) => (
                <div key={item.layer} className="bg-card border border-border/60 p-4 rounded-xl space-y-1.5 hover:border-primary/40 transition-colors">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-mono text-primary uppercase tracking-wider font-semibold">{item.layer}</span>
          ...
          <div className="bg-card border border-border/60 p-6 rounded-2xl space-y-6 flex flex-col justify-between shadow-sm">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Every event moves through strict state transitions, keeping ticket counts, payments, and gate passes synchronized across all nodes.
              </p>
            </div>

            <div className="space-y-3 my-2">
              {stateMachine.map((sm) => (
                <div key={sm.state} className="flex items-start gap-3 bg-muted/40 p-2.5 rounded-lg border border-border/40 text-xs">
