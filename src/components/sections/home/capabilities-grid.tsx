import { IconBrandWhatsapp, IconCalendarEvent, IconChartDots, IconQrcode, IconUsersGroup, IconWallet } from "@tabler/icons-react";
...
export function CapabilitiesGrid() {
  const capabilities = [
    ...
    },
    {
      ...
      highlights: ["Multiple Gateway Integrations", "Organizer Wallet & Split Ledgers", "Coupons, Discounts & Referrals", "Automated Refund Workflow"],
    ...
    },
    {
      ...
      highlights: ["Real-Time Gate Attendance Stream", "Revenue & Sales Conversion Charts", "Referral & Coupon Performance", "Exportable PDF/Excel Reports"],
    ...
    },
    {
      ...
      highlights: ["Role-Based Access (Admin/Volunteer/Check-in)", "Reusable Event & Email Templates", "Multi-Event Campus Hub", "Audit Trail & Action Logs"],
      badgeColor: "bg-indigo-500/10 text-indigo-600 border-indigo-500/20",
    },
  ];

  return (
    <section id="capabilities" className="py-20 md:py-28 bg-muted/20 border-b border-border/40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <h2 className="text-3xl sm:text-5xl font-extrabold text-foreground tracking-tight">Engineered for the Full Event Spectrum</h2>
          <p className="text-muted-foreground text-base sm:text-lg">
            Orgatick is not merely an event listing page. It provides the deep backend systems required to manage registrations, money, entry gates,
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {capabilities.map((cap) => {
            const Icon = cap.icon;
            return (
              <Card key={cap.title} className="bg-card border border-border/60 shadow-sm hover:shadow-md transition-all flex flex-col justify-between group">
                <CardHeader className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full border ${cap.badgeColor}`}>{cap.category}</span>
                  </div>
                  <CardTitle className="text-xl font-bold text-foreground">{cap.title}</CardTitle>
                  <CardDescription className="text-xs sm:text-sm text-muted-foreground leading-relaxed">{cap.description}</CardDescription>
                </CardHeader>
